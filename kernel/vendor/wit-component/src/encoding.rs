...
impl<'a> EncodingState<'a> {
    fn encode_core_modules(&mut self) {
        ...
        for (name, adapter) in self.info.adapters.iter() {
            ...
            // Without the `metadata` feature the adapter name is not embedded (debug-only,
            // informational); the module bytes are otherwise unchanged.
    ...
    fn encode_exports(&mut self, module: CustomModule) -> Result<()> {
        ...
        let mut interface_func_core_names = IndexMap::default();
        let mut world_func_core_names = IndexMap::default();
    ...
    fn encode_shim_function(
        ...
        let mut func = wasm_encoder::Function::new(core::iter::empty());
...
impl ComponentEncoder {
    ...
    pub fn module(mut self, module: &[u8]) -> Result<Self> {
        ...
        self.module = {
            #[cfg(feature = "metadata")]
            {
                if let Some(producers) = &self.metadata.producers {
                    producers.add_to_wasm(&wasm)?
                } else {
                    wasm.to_vec()
                }
            }
            // Without `metadata`, producers are never decoded (always `None`), so the core
            // module is carried through unchanged (no informational producers section).
            #[cfg(not(feature = "metadata"))]
            {
                wasm.to_vec()
            }
    ...
    pub fn encode(&mut self) -> Result<Vec<u8>> {
        ...
        let mut state = EncodingState {
            ...
            adapter_modules: IndexMap::default(),
            adapter_instances: IndexMap::default(),
            type_encoding_maps: Default::default(),
