...
// Without `std` there is no `thread_local!`, so the single pointer lives in a
// per-thread slot provided by the embedder through the custom platform layer
// (`wasmtime_concurrent_tls_{get,set}`), exactly like the existing
// `wasmtime_tls_{get,set}` slot the trap handlers use. The shim below exposes
// the same `.with(|cell| ...)` shape as the `thread_local!` above so the rest
// of this module is identical in both builds.
#[cfg(not(feature = "std"))]
use nostd_storage::STORAGE;

#[cfg(not(feature = "std"))]
mod nostd_storage {
    use super::SetStorage;
    ...
    impl Storage {
        pub fn with<R>(&'static self, f: impl FnOnce(&Slot) -> R) -> R {
            f(&Slot)
        }
    }

    impl Slot {
        pub fn get(&self) -> Option<NonNull<SetStorage>> {
            NonNull::new(sys::concurrent_tls_get().cast::<SetStorage>())
        }

        pub fn set(&self, value: Option<NonNull<SetStorage>>) {
            let ptr = value.map_or(core::ptr::null_mut(), NonNull::as_ptr);
            sys::concurrent_tls_set(ptr.cast::<u8>());
        }

        pub fn replace(
            &self,
            value: Option<NonNull<SetStorage>>,
        ) -> Option<NonNull<SetStorage>> {
            let prev = self.get();
            self.set(value);
            prev
        }
