...
pub fn encode_component(resolve: &Resolve, package: PackageId) -> Result<ComponentBuilder> {
    ...
    // The package-metadata custom section serializes the WIT package via wit-parser's
    // serde path (serde_json, std-only). It is only produced by `wit_component::encode`
    // (a full-WIT-package encoder eo9-component does not use); the component-metadata path
    // eo9-component relies on goes through `encode_world`, which does not touch it. Gated so
    // the no_std kernel build does not pull serde_json.
    #[cfg(feature = "wit-package-metadata")]
    {
...
impl Encoder<'_> {
    ...
    fn encode_interface(&mut self, id: InterfaceId) -> Result<ComponentType> {
        ...
        // roundtripping.
        let mut interfaces = IndexSet::default();
        ...
        let mut used_names = IndexSet::default();
...
impl InterfaceEncoder<'_> {
    ...
    fn encode_instance(&mut self, interface: InterfaceId) -> Result<u32> {
        self.push_instance();
        let iface = &self.resolve.interfaces[interface];
        let mut type_order = IndexSet::default();
        for (_, id) in iface.types.iter() {
