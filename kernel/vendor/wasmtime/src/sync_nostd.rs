...
///
/// Mirrors the role of `std::sync::TryLockError` for the `no_std` mutex below;
/// callers in this crate only ever `.unwrap()`/`.ok()` the result, so the
/// concrete shape is intentionally minimal.
#[derive(Debug)]
pub struct WouldBlock;

/// A `no_std` stand-in for `std::sync::Mutex` with the same call-site shape
/// (`lock()`/`try_lock()` returning `Result`s and a nameable [`MutexGuard`]).
///
/// Like the rest of this module it does not block: `try_lock` reports
/// contention as [`WouldBlock`] and `lock` panics on contention, which keeps it
/// correct (never two guards at once) in the face of actual multiple threads
/// while remaining usable on single-threaded embeddings without host lock
/// support.
