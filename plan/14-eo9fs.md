# 14 — Native filesystem `eo9fs` (`crates/eo9fs-core`, guest provider)
...
and compilation cache" (hash/immutability synergy), "Packaging and submodules" (`eo9fs.mkfs` as a sibling
binary world).

## Deliverables
- `crates/eo9fs-core` — `#![no_std]` + `alloc`, target-independent (usable from host tests, the guest
...
- Later milestones: the guest provider component (imports `eo9:disk` + time/entropy, exports `eo9:fs`
  including hash queries and `open-exec`), an `eo9fs.mkfs` sibling tool, usermode end-to-end, kernel adoption
...
## Milestones
1. **Core library + format.** `FORMAT.md`, eo9fs-core with create/mount/read/write/mkdir/list/stat/remove,
   ...
   clean.
2. **Provider component + `eo9fs.mkfs`** (with plan 07's provider support).
3. **Usermode end-to-end** over `disk.mem` and the file-backed disk (with plans 04/11); store-on-eo9fs
   evaluation with plan 06.
4. **Kernel adoption** (plan 12): boot disk formatted as eo9fs, read-only first, then read-write.

## Notes / constraints
- Keep eo9fs-core free of wasm/wasmtime/OS types; all I/O goes through the `BlockDevice` trait (sync trait in
