...
// Re-export the standard library's mutex for use via `crate::sync` so that
// code shared with `no_std` builds (which get the `sync_nostd.rs` equivalent)
// can name a single type. Poisoning behavior is unchanged for `std` users.
