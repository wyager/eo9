...
/// Minimal stand-in for `std::io::Cursor<Vec<u8>>`, used by the host-to-host
/// buffer plumbing in this module.
///
/// Implemented locally (only the handful of methods this module needs) so the
/// concurrent machinery does not depend on `std::io` and stays usable in
/// `no_std` builds of the crate.
struct VecCursor {
    inner: Vec<u8>,
    position: u64,
}
...
pub struct Destination<'a, T, B> {
    ...
    buffer: &'a mut B,
    host_buffer: Option<&'a mut VecCursor>,
                            ...
                            tls::get(|store| {
                                ...
                                let mut host_buffer =
                                    if let ReadState::HostToHost { buffer, .. } = &mut transmit.read {
                                        Some(VecCursor::new(mem::take(buffer)))
                                    } else {
                                        None
                                    };
        ...
        ReadState::GuestReady {
            ...
            if guest_offset < count {
                if T::MAY_REQUIRE_REALLOC {
                    ...
                    rx.await
                        .map_err(|_| format_err!("oneshot channel was canceled"))?
                } else {
