...
impl FiberStack {
    pub fn new(size: usize, zeroed: bool) -> Result<Self> {
        ...
        // The *top* of the stack (`base + len`) must be aligned to `STACK_ALIGN`
        // too: it is where execution starts and where the fiber's saved-state
        // words live, and every frame on the stack inherits its alignment. The
        // raw allocation's end carries whatever alignment the global allocator
        // happened to give it (the storage is a byte vector), so round the
        // usable length down to keep the top aligned. On platforms with mmap
        // the top is page-aligned for free; here it is not.
        let len = len & !(STACK_ALIGN - 1);
        Ok(FiberStack {
            storage,
            base: BasePtr(base),
            len,
