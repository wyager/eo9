...
impl FliConstant {
    ...
    pub(crate) fn maybe_from_u64(ty: Type, imm: u64) -> Option<Self> {
        ...
        Some(match (ty, value) {
            ...
            // Eo9 no_std: `f64::powi` is a std-only method; these are exact powers of two,
            // so spell them as exact divisions instead (kernel/vendor/README.md).
            (_, f) if f == 1.0 / 65536.0 => Self::new(2),
            (_, f) if f == 1.0 / 32768.0 => Self::new(3),
            (_, f) if f == 1.0 / 256.0 => Self::new(4),
            (_, f) if f == 1.0 / 128.0 => Self::new(5),
