...
use wasmparser::{
    ...
    component_types::{
        ...
        ComponentInstanceType, ComponentItem, ComponentType, ComponentValType,
    },
...
impl ComponentInfo {
    /// Creates a new component info by reading the given WebAssembly component
    /// bytes from a streaming reader. `std`-only convenience wrapper; the
    /// no_std parsing path is [`ComponentInfo::from_bytes`].
    #[cfg(feature = "std")]
    fn from_reader(mut reader: impl Read) -> Result<Self> {
        let mut buffer = Vec::new();
        reader
            .read_to_end(&mut buffer)
            .map_err(|e| anyhow!("failed to read component bytes: {e}"))?;
        Self::from_bytes(&buffer)
    }

    /// Creates a new component info by parsing the given complete WebAssembly
    /// component bytes. Works without `std` (no streaming reader required): the
    /// whole input is present, so the parser is driven with `eof = true`.
    fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        loop {
            let chunk = cur.parse(&buffer, true)?;
            let (payload, consumed) = match chunk {
                Chunk::NeedMoreData(_) => {
                    // `eof = true` was passed, so the parser never asks for more
                    // data on a complete, well-formed input.
                    bail!("unexpected end of input while parsing a component");
                }
    ...
    fn decode_wit_v2_package(&self) -> Result<(Resolve, PackageId)> {
        ...
        for (_, item) in self.externs.iter() {
            ...
            let name = match component.exports[name].ty {
    ...
    fn decode_component(&self) -> Result<(Resolve, WorldId)> {
        ...
        for (_name, item) in self.externs.iter() {
            match item {
                Extern::Import(name) => {
                    decoder.decode_component_import(name, world, &mut fields)?
                }
...
/// Decode for incremental reading. `std`-only; the no_std entry point is
/// [`decode`], which takes a complete byte slice.
#[cfg(feature = "std")]
pub fn decode_reader(reader: impl Read) -> Result<DecodedWasm> {
    let info = ComponentInfo::from_reader(reader)?;
    decode_component_info(info)
}
...
pub fn decode(bytes: &[u8]) -> Result<DecodedWasm> {
    let info = ComponentInfo::from_bytes(bytes)?;
    decode_component_info(info)
}

fn decode_component_info(info: ComponentInfo) -> Result<DecodedWasm> {
...
impl WitPackageDecoder<'_> {
    ...
    fn decode_component_import<'a>(
        ...
        let item = self.types.as_ref().component_item_for_import(name).unwrap();
    ...
    fn decode_component_export<'a>(
        ...
        let item = types.component_item_for_export(name).unwrap();
        let (name, item) = match item.ty {
            ComponentEntityType::Func(i) => {
                let ty = &types[i];
                let func = self
