...
use {
    ...
    crate::{IndexMap, IndexSet},
    alloc::collections::BTreeMap,
    anyhow::{Context, Result, anyhow, bail},
    core::{fmt::Debug, hash::Hash, iter},
...
fn make_env_module<'a>(
    ...
    let mut import_map = IndexMap::default();
    let mut function_count = 0;
...
fn resolve_exporters<'a>(
    ...
    let mut exporters = IndexMap::<_, Vec<_>>::default();
    for metadata in metadata {
        for export in &metadata.exports {
            exporters
                .entry(&export.key)
...
fn topo_sort(count: usize, dependencies: &IndexMap<usize, IndexSet<usize>>) -> Result<Vec<usize>> {
    let mut sorted = IndexSet::default();
...
fn find_dependencies(
    ...
    let mut dependencies = IndexMap::<_, IndexSet<_>>::default();
    ...
    loop {
        let mut new = IndexMap::<_, IndexSet<_>>::default();
    ...
    pub fn encode(mut self) -> Result<Vec<u8>> {
        ...
        if !missing.is_empty() {
                        ...
                        .all(|(_, export)| export.flags.contains(SymbolFlags::BINDING_WEAK)))
            {
                ...
                // Build the stub module while the metadata borrows are live, then release
                // them before mutating `self.libraries` and moving `self` into `encode`.
                // hashbrown maps lack std's `#[may_dangle]` dropck eyepatch, so these
                // collections otherwise keep `self` borrowed until end of scope. Dropped in
                // dependency order — `missing`/`exporters` borrow `metadata`.
                let stub = make_stubs_module(&missing);
                drop(missing);
                drop(exporters);
                drop(metadata);
                drop(adapter_names);
                self.libraries
                    .push((STUB_LIBRARY_NAME.into(), stub, false));
                return self.encode();
            } else {
                bail!(
                    "unresolved symbol(s):\n{}",
...

        {
            ...
            if !unreachable.is_empty() {
                // Release the `self`-borrowing collections (hashbrown maps lack std's
                // `#[may_dangle]` eyepatch) before mutating `self.libraries` and moving
                // `self` into `encode`. Dropped in dependency order; `unreachable` is owned
                // (cloned names), so it stays.
                drop(reachable);
                drop(dependencies);
                drop(exporters);
                drop(metadata);
