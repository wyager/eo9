...
- **Eagerness is declared at the ABI, not the type.** Middleware providers with synchronous cores declare their eagerness at the canonical-ABI level: their imports are sync-lowered and their exports sync-lifted, while their WIT types remain `async`. Such a component completes every operation within its own activation, so compositions of eager components — attenuators, filters, switches, drivers — execute as direct fused calls at any depth, and an eager component above them never observes a suspension. A component that genuinely parks keeps async lifts; composing it under an eager component is refused by the eager component's typed suspension error, never a hang.
