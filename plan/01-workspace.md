...
## Deliverables
- Three build roots:
  - Host workspace at repo root (`crates/*`, `xtask`), stable Rust.
  ...
  - `kernel/` workspace (no_std, custom targets; may use nightly — pending PLAN.md Q5).
- `rust-toolchain.toml` per workspace; pinned `wasmtime` / `wasm-tools` versions in a single workspace-level
  `[workspace.dependencies]` table so every crate uses the same versions.
- `xtask` (or `justfile`): `build`, `test`, `build-guest`, `build-kernel <arch>`, `qemu <arch>`, `fmt`, `lint`.
- CI skeleton: fmt + clippy + host tests + guest build (+ QEMU job placeholder). Host: Linux + macOS.
