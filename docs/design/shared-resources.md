...
// boundary — the component-argument/detach precedent, shellexec.rs:2445)
```
...
* **Nothing exports a factory by accident.** Providers are not universally shareable; you make a composition shareable by composing a tiny factory component at the tail — `net.virtio $ net.l4.over-l2 $ l4.factory` (a ~50-line component whose `get` returns its fused import's `default()`) — or by authoring your own factory export with policy in it. The decision to share is a visible, deliberate line in the composition.
