...
//! compares. Every fs operation is an async function, awaited directly from the world's
//! async `main`.
...
eo9_guest::main! {
    async fn main(path: String, contents: String) -> Result<ProgramSuccess, ProgramFailure> {
        if path.is_empty() {
