...
**Policies are programs.** Where an Eo9 API takes a non-trivial *decision* parameter — a restart policy, a device filter, an access rule — it takes a **component**, not a configuration enum: a tiny provider exporting the policy interface, required to import nothing. Purity is therefore enforced by the capability system rather than promised by documentation — a policy provably cannot read the network, the clock, or anything else. Policies bind at either of the two times everything else in Eo9 binds: composed in at compose time (fused and inlined, for hot paths) or passed as a value at call time (instantiated separately, for cold paths like a restart decision). Configuration *data* remains for what must be legible at a glance — addresses, sizes, names, allow-lists; the line is "does this parameter decide something, or describe something?"
...
**Binary or provider, never both.** Every module is exactly one kind. A *binary* exports `main(args)` and is run; a *provider* exports interfaces plus `configure(args)` and is composed. The two argument surfaces stage differently: a binary's `main` args are bound at run time and may differ each run, while a provider's `configure` args are bound at compose time. A provider takes config the same type-directed way a binary takes flags — `virtualfs --dir /tmp/sandbox` binds `configure(dir: string)` — and because that config is usually a compile-time constant, the compose-and-compile step specializes the provider with it and inlines, so even a configured layer stays zero-overhead. An invalid value fails at `configure`, before the consumer ever runs. Binding those flags is itself an operation of the component algebra — `configure : provider × args → provider` — which bakes the constants in and re-exports only the API, so the consumer can neither observe nor re-run the configuration. `configure` returns the provider's root capability handle(s) — the `x-impl` resources its exported interfaces operate on — and the consumer-facing `default()` accessor hands out exactly the handle that `configure` produced. Configuration→capability is therefore explicit in the types: an unconfigured provider has no handle to give, so "used before configured" is unrepresentable rather than a convention. Concretely, the export shape encodes whether configuration is required: a provider with a sensible default exports its API directly (the default baked in, with a config interface only as an optional override), while a provider that genuinely requires arguments exports *only* its config interface — so composing it unconfigured is a compose-time type mismatch (the consumer's API import has nothing to bind to), reported with a hint to apply `configure(…)`, never a runtime failure. `describe` reads the requirement off the same shape, so tooling can mark required provider arguments without any side-channel metadata. A configurable provider used without `configure` therefore either self-binds a documented default (the standard stubs do: the empty memfs, the 2000-01-01 frozen instant, a 1 ms fuzzy granularity, the `0xE09` seed) or fails with that typed, pre-run refusal naming the missing configuration — it never traps. (Encoding note: `configure` lives in a small exported config interface rather than as a bare world-level export, because a component cannot mint handles of a resource type it merely `use`s — see the packaging example below.)

**How a configured provider is realized.** A configured provider re-exports its provider's interfaces unchanged — direct aliases, so resource types keep their identity and there is no per-call indirection — and adds one runtime-contract export, `eo9:rt/configured`; its parameterless `bind` applies the baked configuration. Executors call `bind` once after instantiation, before first entry; `$`/`&` propagate the entrypoint (merging provider-first when both operands carry one), so the outermost component of any composition reaches every nested configured provider. Like the diagnostics rider, `eo9:rt/configured` is not a capability: it grants nothing, guests never import it, and allow-lists never name it. A configuration the provider rejects — an invalid baked value — is returned through `bind` as the provider's own error text and surfaces as a typed pre-run refusal ("compose-time configuration refused: …"); it never traps, and the program is never entered. One obligation falls on embedders: an executor that instantiates a configured artifact and never calls `bind` gets the provider's documented defaults instead of the baked configuration — conforming Eo9 executors (usermode, kernel, browser) all make the call, and any other embedding must too.
...
- **Boundaries are honestly async; the runtime makes the fast case fast.** Every operation that can wait in practice is declared *and bound* async — no component adopts synchronous bindings because its current substrates happen to complete eagerly. A component that provably never waits (a pure attenuator, filter, or policy) may declare eagerness at the canonical-ABI level (sync-lowered imports, sync-lifted exports) as a *measured optimization*; the declaration is enforced, not trusted — a sync-bound component that attempts to wait fails with a typed error, never undefined behavior. The runtime direction is to complete non-waiting async calls inline where entry is legal (first-poll of guest callees, exactly as host calls are first-polled today), so honest asyncness does not tax compositions whose layers complete eagerly. And everything that parks, parks bounded: an await across a trust boundary without a deadline or kill-scope is a liveness bug. The mirror rule on the
executor side: liveness is event-driven, never timer-driven — a deadline timer whose event *is* time (sleep,
restart delays, bound expiry) is legitimate; a periodic backstop that exists so progress happens is a
confession. Backstops may remain as detectors during bring-up, but every backstop firing that discovers
actionable work no event delivered is a high-priority bug, and the end state carries none.
...
**Multi-instance imports and type identity.** Resource types are nominal *per implementer*: a WIT interface is a template, every component that exports it mints its own resource types, and an import is an abstract placeholder bound to some exporter at composition time. So that two *independent* providers can be bound to two slots of the same interface without their handles unifying, **an API's root-handle resource (`fs-impl`, `disk-impl`, …) is declared in the API interface itself**, not in the types-only sibling — then every named import of the interface mints its own abstract root-handle type (exactly as interface-local resources like `file` already do), and the type system keeps one slot's handles from ever being passed to the other. The types-only sibling interface keeps the plain data types (records, enums, errors), which carry no identity. Anything that only needs to *name* the root handle (`-optional` flavors, `none`/`deny` stubs, config interfaces) does so with a `use` of the API interface — a types-only use, which carries no authority and is reported as such by `describe`. (WIT today cannot re-bind a named import's `use` dependencies to a chosen sibling import, which is why per-slot identity must come from the API interface itself rather than from importing the types interface once per slot.) `fs.overlay` is the canonical multi-instance consumer: its `upper` and `lower` slots each mint their own root-handle type, and it exports its own for consumers above it.
...
**Optional capabilities are typed, not metadata.** Every API declares its root resource and an accessor for obtaining it (this is also how a program gets its `disk-impl`/`net-impl` handle in the first place); the root resource is declared in the API interface itself (see *Multi-instance imports*), and the `-optional` flavor and the stubs name it through a types-only `use` of that interface — which imports the type, not the authority. Tooling mechanically derives an `-optional` flavor whose accessor returns `option`:
...
- **Services and detachment (`eo9:svc`).** A program holding the *detach* capability may hand a composed child to the service registry and walk away: the child is reparented (the one exception to kill-cascade) and keeps running for the registry's lifetime, which is the root process's configuration — the usermode CLI binds it to the `eo9` process; the kernel binds it to the machine. Detach is never in the default child grant: outliving your creator is authority. **Capability soundness:** a detached child runs with exactly what its detacher composed into it plus the registry's log capture and rt riders — never the registry's authority — so handing a child off can never escalate it. *Kernel refinement (operator trust):* the kernel registry additionally links the ambient kernel roots (time, entropy, io) and the boot-granted operator roots (pci/platform/gfx/kexec/console-sink) into every service — exactly the set the console session's own children link, gated by the same boot tokens. Operator-authored services (an init config line, a console `detach`) are console-equivalent trust: a config line can do exactly what a console line can do — same operator, same authority — and the invariant holds in its real form, a service never exceeding what its detacher could run in the foreground. The usermode registry keeps the composed-only rule (its roots are ambient unix providers, not boot grants); fs/exec/svc/net are never linked to a service on either target. **Restart policies are programs:** detach requires a restart-policy component (a pure provider exporting `eo9:svc/restart-policy`); the registry instantiates it per decision, and a policy that traps or stalls reads as give-up, never a hang. A killed service never consults its policy: stop means stop. The *services* capability (inspect/log/stop/clear) is separable from detach, and the boot pipeline's `init` is just an ordinary program holding both.
