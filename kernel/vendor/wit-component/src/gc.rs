...
use core::{convert::Infallible, mem, ops::Deref};
...
pub fn run(
    ...
    let mut not_required = IndexSet::default();
...
struct Module<'a> {
    ...
    producers: Option<crate::Producers>,
...
impl<'a> Module<'a> {
    ...
    fn parse_producers_section(&mut self, section: &CustomSectionReader<'a>) -> Result<()> {
        let producers =
            crate::Producers::from_bytes(section.data(), section.data_offset())?;
