...
**Multi-instance imports and type identity.** Resource types are nominal *per implementer*: a WIT interface is a template, every component that exports it mints its own resource types, and an import is an abstract placeholder bound to some exporter at composition time. Consequently a world that wants two *independent* instances of a capability must import the interface's types **per slot** (e.g. `system-fs-types` + `system-fs`, `scratch-fs-types` + `scratch-fs`): each slot is then its own abstract type of the same shape, two different providers can be bound without their handles unifying, and the type system keeps one slot's handles from ever being passed to the other. Importing a types interface once and `use`-ing it from several slots asserts the opposite — that all those slots share one implementation — which is right only when they really are served by a single instance (e.g. two views handed out by the same provider, or root slots all satisfied by the OS core). `with … as …` wires a provider to a slot by binding its exported types+API pair to that slot's pair. `fs.overlay` is the canonical multi-instance consumer: per-slot types for `upper` and `lower`, its own exported types for consumers above it.
