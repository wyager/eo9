...
//! `component-algebra` interface; `compile` runs a plain program from its pre-AOT'd artifact
//! and compiles a fused composition (`$`/`&`/`configure` results) **inside the blob** with the
//! same vendored Cranelift + wasmtime compile layers the bare-metal kernel uses on-target,
//! emitting Pulley bytecode (the `inblob-codegen` feature, on by default — fully client-side,
//! no server); `task` runs a child program to completion (single-child, no concurrency: eosh
//! awaits each command fully before reading the next line).
//!
...
/// Compile a fused component's executable bytes inside the blob — the same vendored
/// Cranelift + wasmtime compile layers the bare-metal kernel uses for on-target codegen,
/// emitting Pulley bytecode — and return the serialized artifact, ready for the same
/// deserialize-and-run path a pre-AOT'd program uses. Fully client-side.
#[cfg(feature = "inblob-codegen")]
fn compile_in_blob(component: &Component) -> Result<Vec<u8>> {
    let engine = engine(false)?;
    let bytes = component.executable_bytes();
    let compiled = WtComponent::new(&engine, &bytes)?;
    compiled.serialize()
}
        ...
        |mut store: StoreContextMut<'_, WebState>,
         ...
         -> Result<(std::result::Result<Resource<ImageRes>, WitCompileError>,)> {
            ...
            match entry.artifact {
                ...
                // No precompiled artifact: a fused composition (`$`/`&`/`configure`). Compile
                // it inside the blob — the same vendored Cranelift + wasmtime compile layers
                // the bare-metal kernel uses for on-target codegen, emitting Pulley bytecode —
                // and run the result like any other image. Fully client-side.
                #[cfg(feature = "inblob-codegen")]
                None => match compile_in_blob(&entry.component) {
                    Ok(image) => {
                        let arg_specs = info.args.into_iter().map(|a| (a.name, a.ty)).collect();
                        let rep = store.data_mut().exec().put_image(ImageEntry {
                            artifact: image,
                    ...
                    Err(error) => Ok((Err(WitCompileError::Codegen(std::format!(
                        "in-blob codegen failed: {error}"
                    ))),)),
                },
                // Without the in-blob compiler (feature `inblob-codegen` off — the size-
                // measurement build), a fused composition gets the honest refusal.
                #[cfg(not(feature = "inblob-codegen"))]
                None => Ok((Err(WitCompileError::Codegen(
                    "composition needs the compiler, which is not built into this blob (feature \
                     `inblob-codegen` is off); native Eo9 and the bare-metal kernel compile it \
                     on-target"
                        .to_string(),
                )),)),
            }
